...
RISK_HINTS = [
    r"http://", r"android:debuggable=\"true\"", r"android:allowBackup=\"true\"", r"android:usesCleartextTraffic=\"true\"",
    r"MODE_WORLD_READABLE", r"MODE_WORLD_WRITEABLE", r"MD5", r"SHA-1", r"AES\/ECB", r"setJavaScriptEnabled\(true\)",
    r"cleartextTrafficPermitted=\"true\"",
]
POSITIVE_TLS_HINTS = [r"android:usesCleartextTraffic=\"false\"", r"NetworkSecurityConfig.*cleartextTrafficPermitted=\"false\""]
...
def parse_manifest(source_root: Path) -> ManifestInfo:
    manifest = next(iter(source_root.rglob("AndroidManifest.xml")), None)
    ...
    if manifest and manifest.exists():
        try:
            ...
            pass
    return ManifestInfo(manifest, allow_backup, debuggable, uses_cleartext, ns_cfg_path, n_exported, exported_details, target_sdk)
...
def mobsf_text_hints(json_file: Path) -> set[str]:
    ...
    hints = set()
    for k in ["android:debuggable", "allowbackup", "webview", "http://", "https://", "aes/ecb", "md5", "cleartexttrafficpermitted", "setjavascriptenabled(true)", "usescleartexttraffic"]:
        if k in s:
            hints.add(k)
    return hints
...
def collect_evidence(reports_dir: Path, source_root: Path) -> Evidence:
    ...
    ns_flags = parse_network_security_config(source_root, mani.network_security_config_path) if mani.network_security_config_path else {}
    return Evidence(sarif_rules, mobsf_hints, trivy_summary, mani, ns_flags)
...
def requirement_seems_related(req: Dict[str, Any], caps: set[str]) -> bool:
    ...
    conds = [("saml", "saml"), ("webview", "webview"), ("biometric", "biometric"), ("bluetooth", "ble"), ("ble", "ble"), ("nfc", "nfc"), ("camera", "camera"), ("location", "location"), ("oauth", "oauth")]
...
def summarize_reports_for_ai(ev: Evidence) -> str:
    ...
    if ev.trivy_summary:
        lines.append(f"Trivy severity tags: {ev.trivy_summary.get('severity', {})}")
    m = ev.manifest
    lines.append(f"Manifest: allowBackup={m.allow_backup} debuggable={m.debuggable} usesCleartextTraffic={m.uses_cleartext_traffic} exported(IF)={m.n_exported_with_intentfilter}")
    if ev.ns_flags:
...
RULES:
- \"N/A\" if the capability clearly does not exist in app context.
- \"NO\" if there is contradictory evidence vs the requirement.
- \"YES\" only if requirement applies and there is positive evidence or absence of contradictions.
- \"MANUAL\" when runtime behavior or evidence is insufficient or ambiguous.
...
def llm_decide(puid: str, req: Dict[str, Any], app_ctx: str, code: CodeHints, ev: Evidence, timeout_s: int) -> Tuple[str, str, Dict[str, Any]]:
    if not AI_RUNTIME_AVAILABLE or not os.getenv("OPENAI_API_KEY"):
        return ("N/A", "AI runtime not available", {})
    runtime = AIRuntime(task=os.getenv("AI_TASK", "").strip() or "ai_correlate", api_key=os.getenv("OPENAI_API_KEY"))
    ...
    start = time.time()
    try:
        rsp = runtime.create(model=model, input=prompt, max_output_tokens=max_out, reasoning={"effort": reasoning_effort})
        if time.time() - start > timeout_s:
            return ("MANUAL", "LLM timeout -> manual review suggested", {"manual_steps": ["Review requirement on device or emulator and inspect logs"]})
        text = getattr(rsp, "output_text", "") or ""
    except Exception as e:
        return ("MANUAL", f"LLM error: {e}", {"manual_steps": ["Review manually because context is insufficient"]})
    obj = safe_json_extract(text or "")
    if not obj or "decision" not in obj:
        return ("MANUAL", "Unparseable LLM output", {"manual_steps": ["Review manually because output was invalid"]})
    decision = str(obj.get("decision", "MANUAL")).strip().upper()
    ev_field = obj.get("evidence", "")
    evidence_text = " | ".join(str(x) for x in ev_field[:6]) if isinstance(ev_field, list) else str(ev_field or "").strip()
    extras: Dict[str, Any] = {}
...
def heuristic_decide(req: Dict[str, Any], code: CodeHints, ev: Evidence) -> Tuple[str, str, bool]:
    ...
    if _req_has(text, ["https", "tls", "cleartext", "http only forbidden", "no cleartext"]):
        ...
        if cleartext:
            return ("NO", "Cleartext permitted or used", False)
        ...
        return ("MANUAL", "No contradiction visible, but runtime corroboration is still needed", True)
    ...
    if _req_has(text, ["backup", "allowbackup"]):
        ...
        return ("MANUAL", "allowBackup not declared -> review needed", True)
    ...
    if _req_has(text, ["debuggable", "release build debuggable", "no debuggable in prod"]):
        ...
        return ("MANUAL", "debuggable not declared -> review buildTypes", True)
    ...
    if _req_has(text, ["export", "exported", "implicit exported", "component exposure"]):
        ...
        return ("YES", "No exported components with intent-filter", False)
    ...
    if _req_has(text, ["md5", "sha-1", "weak hash", "weak crypto", "ecb"]):
        if any(k for k in rules if re.search(r"(md5|sha-1|weak|ecb)", k, re.I)) or any("MD5" in m or "AES/ECB" in m for m in code.matches):
            return ("NO", "Weak crypto patterns or SARIF rules detected", False)
    ...
    if _req_has(text, ["pii", "logs", "no sensitive info in logs", "logging"]):
        if any("Log.d" in m or "Log.i" in m for m in code.matches):
            return ("MANUAL", "Runtime review of logcat is needed", True)
        return ("MANUAL", "No static evidence is sufficient; runtime validation is needed", True)

    caps_text_pairs = [("saml", "saml"), ("webview", "webview"), ("biometric", "biometric"), ("nfc", "nfc"), ("bluetooth", "ble"), ("ble", "ble"), ("camera", "camera"), ("location", "location"), ("oauth", "oauth")]
    ...
    if any(k in text for k in ["tls", "encrypt", "permission", "session", "token", "cookie", "audit", "pinning"]):
        return ("MANUAL", "Static evidence is insufficient to conclude", True)
...
def write_markdown_summary(path: Path, totals: Dict[str, int], notes: List[str]) -> None:
    md = ["# Audit Summary\n", f"- Total requirements: {totals['total']}", f"- YES: {totals['YES']}", f"- NO: {totals['NO']}", f"- N/A: {totals['N/A']}", f"- MANUAL: {totals['MANUAL']}\n"]
    if notes:
        md.append("## Notes\n")
...
def write_docx_summary(path: Path, totals: Dict[str, int], template: Optional[Path]) -> None:
    ...
    if not template:
        ...
        p = doc.add_paragraph(f"Total requirements: {totals['total']} | YES: {totals['YES']} | NO: {totals['NO']} | N/A: {totals['N/A']} | MANUAL: {totals['MANUAL']}")
        p.runs[0].font.size = Pt(12)
    else:
        doc.add_paragraph(f"Total requirements: {totals['total']} | YES: {totals['YES']} | NO: {totals['NO']} | N/A: {totals['N/A']} | MANUAL: {totals['MANUAL']}")
    doc.save(path.as_posix())
...
def main() -> int:
    ...
    ap.add_argument("--preselect", action="store_true")
    ap.add_argument("--mark-manual-when-uncertain", action="store_true")
    ap.add_argument("--use-llm", choices=["never", "uncertain", "always"], default="never")
    ap.add_argument("--llm-timeout", type=int, default=25)
    ap.add_argument("--app-context", type=Path, default=None)
    ap.add_argument("--catm-template", type=Path, default=None)
    ap.add_argument("--summary-template", type=Path, default=None)
    ap.add_argument("--verbose", action="count", default=0)
    args = ap.parse_args()

    logging.basicConfig(level=logging.WARNING - min(args.verbose, 2) * 10, format="%(levelname)s | %(message)s")
    ...
    for idx, r in enumerate(selected, start=1):
        ...
        if need_llm and AI_RUNTIME_AVAILABLE and os.getenv("OPENAI_API_KEY"):
