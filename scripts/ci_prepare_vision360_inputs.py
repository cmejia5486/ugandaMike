...
def main() -> int:
    ...
    for expected_zip, spec in REQUIRED.items():
        ...
        # Special: openMRS.zip should be found by filename first; if not found, try any zip named similarly.
        if expected_zip == "openMRS.zip":
            cand = None
            # exact basename match first:
            for p in files:
                if os.path.basename(p).lower() in {"openmrs.zip", "openmrs-zip.zip", "openmrs_source.zip"} and is_zip(p):
                    cand = p
                    break
            # otherwise any file named openMRS.zip anywhere
            if cand is None:
                cand = find_best_zip_candidate(files, "openMRS.zip", [])
            if cand is None:
    ...
    if missing:
        ...
        print("  - openMRS.zip from package_source_zip job", file=sys.stderr)
        return 2
